...
const webpack = (options, callback) => {
	...
	if (Array.isArray(options)) {
		...
		//结合webpack内部的默认配置，来构建形成新的配置
		options = new WebpackOptionsDefaulter().process(options);

		//创建compiler，相对于整个webpack的调度中心，如何如执行内部逻辑
		...
		//传入用户所有的插件逻辑，并且执行传入compiler执行监听逻辑，后续在特定的通知下执行对应的逻辑
		...
		//各种webpack内部默认执行的插件的挂载，以及内部处理逻辑的各种插件挂载
