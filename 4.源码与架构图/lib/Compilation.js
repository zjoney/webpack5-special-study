...
class Compilation extends Tapable {
	...
	buildModule(module, optional, origin, dependencies, thisCallback) {
		...
		//调用对应的normalModule的build构建它内部的内容
		module.build(
			this.options,
			this,
					...
					factory.create(
						...
						(err, dependentModule) => {
							...
							if (addModuleResult.build) {
								//创建好了NormalModule之后，需要开始编译处理内部的内容了
								this.buildModule(
									dependentModule,
									isOptional(),
									module,
		...
		this.semaphore.acquire(() => {
			...
			moduleFactory.create(
				...
				(err, module) => {
					...
					onModule(module);//把module添加到对应的entries数组中维护，对于入口文件
	...
	//生成chunk，对chunks的一系列优化，并且最后输出代码
	seal(callback) {
		/**
		 * 首先编译入口文件，把每一个entry生成一个chunk，还有对应的spilteCode也会生成chunk
		 *根据module.dependencies来把所对应的依赖添加到chunk中
		 *如果某个module动态引入，创建一个新的chunks
		 */
		...
		for (const preparedEntrypoint of this._preparedEntrypoints) {
			...
			const chunk = this.addChunk(name);//生成对应的chunk
	...
	createChunkAssets() {
		...
		for (let i = 0; i < this.chunks.length; i++) {
			...
			try {
				const template = chunk.hasRuntime()
					? this.mainTemplate//带有webpack的启动代码的模板，渲染入口chunk
					: this.chunkTemplate;//渲染非入口chunk

				//寻找不同的Template来渲染最终代码
				...
				for (const fileManifest of manifest) {
					...
					if (
						...
						source = fileManifest.render();//render生成不同的包装代码，
